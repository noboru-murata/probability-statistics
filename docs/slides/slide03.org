...
# #+DATE: Time-stamp: <2020-05-25 23:37:47 mura>
:reveal:
...
* 演習
  ...
  - 起点 $0$ から周に沿った長さ $l$ の位置に対して，
    以下のような歪んだ目盛り $x$ を付けたルーレットの確率密度を求めよ．
    1. $x=\sqrt{l}$
    2. $x=-\log(l)$
...
** Lebesgue積分
   - 定義
     ...
     #+reveal: split
     #+begin_quote
     このとき $\mu$ をLebesgue測度とし，
     単関数 $f_n(x)$ の *Lebesgue積分* を
     #+begin_src latex
       \begin{equation}
         \int_K f_n(x)dx
...
** 2つの積分の違い
   ...
   - Lebesgue積分の場合
     ...
     #+begin_src latex
       ...
       \end{equation}
     #+end_src
     となるのであれば，Lebesgue測度の性質より
     #+begin_src latex
       \begin{equation}
         \int f_{n}(x)dx 
         = \alpha_{i}\times\mu(\text{無理数})+\alpha_{j}\times\mu(\text{有理数})
         = \alpha_{i}\cdot1+\alpha_{j}\cdot0=\alpha_{i}
       \end{equation}
     #+end_src
     となるが，
     $n\to\infty$ のとき 
...
** 積分の一致する条件
   - 定理
     #+begin_quote
     有界閉区間 $K$ で $f$ が連続なら，
     $K$ 上の $f$ のLebesgue積分の値はRiemann積分の値と一致する．
     #+end_quote
     - 応用上重要な問題に現れる
      「あまり特殊でない関数」では
       ほとんど一致すると考えてよい
