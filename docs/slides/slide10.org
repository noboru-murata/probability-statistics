...
# Time-stamp: <2024-06-19 16:37:13 mura>
:REVEAL:
...
- 定義
  #+begin_quote
  点推定とは
  母数 \(\theta\) を \(X_{1},\dotsc,X_{n}\) の関数
  \begin{equation}
    \hat{\theta}=\hat{\theta}(X_{1},\dots,X_{n})
  \end{equation}
  で推定することで，
...
- 定義
  ...
  観測データ \(X_{1},X_{2},\dots,X_{n}\) が得られる理論上の確率
  \begin{equation}
    L(\boldsymbol{\theta})
...
#+reveal: split
#+begin_leftcol
#+begin_src R :file figs/level_small.png :exports results :results graphics
  shape<-3; rate<-1
  dnull <- function(x)dgamma(x,shape=shape,rate=rate)
  qnull <- function(x)qgamma(x,shape=shape,rate=rate)
  dalt <- function(x)dgamma(x,shape=shape+4,rate=rate)
  qalt <- function(x)qgamma(x,shape=shape+4,rate=rate)
  d <- 0.01
  z <- rbind(c(qnull(0.95), 12), # level small
             c(qnull(0.80), 12)) # level large
  rcol <- c("pink","palegreen")
  if(Sys.info()["sysname"]=="Darwin"){par(family="HiraginoSans-W4")}
  for (i in 1) {
      curve(dnull(x),
            from=0, to=12,
            col="blue", lwd=2, ann=FALSE, axes=FALSE)
      text(2.5,.18,labels="帰無分布")  
      curve(dalt(x), 0, 12,
            col="brown", lwd=2, add=TRUE)
      text(7,.18,labels="ある対立仮説のもとの分布")  
      abline(h=0, col=gray(0.5))
      box()
      x <- seq(min(z[i,]), max(z[i,]), 0.1)
      xx <- c(x,max(z[i,]), min(z[i,]))
      yy <- c(dnull(x), rep(0,2))
      polygon(xx, yy, col=rcol[i])
      yy <- c(dalt(x), rep(0,2))
      polygon(xx, yy, col=adjustcolor(rcol[i],alpha=.3))
      lines(c(z[i,1], z[i,1]),
            c(max(dnull(z[i,1]),dalt(z[i,1]))+d, -d),
...
#+end_leftcol
#+begin_rightcol
#+begin_src R :file figs/level_large.png :exports results :results graphics
  if(Sys.info()["sysname"]=="Darwin"){par(family="HiraginoSans-W4")}
  for (i in 2) {
      curve(dnull(x),
            from=0, to=12,
...
- [[color:green][平均値の検定]]
  ...
  寿命はそれぞれ \(X_{1},X_{2},\dotsc,X_{n}\) であった．
...
- [[color:green][平均値の差の検定]]
  ...
  それぞれの標本平均を計算したところ
  \begin{align}
    X_{1},X_{2},\dotsc,X_{n} & \to \hat\mu^X=\bar{X}\\
    Y_{1},Y_{2},\dotsc,Y_{m} & \to \hat\mu^Y=\bar{Y}
  \end{align}
  となった．
...
- 観測値の確率モデル
  #+begin_quote
  \begin{align}
    X_i&=\theta_{1}+\varepsilon_{1i},\quad i=1,\dotsc,n\\
    Y_j&=\theta_{2}+\varepsilon_{2j},\quad j=1,\dotsc,m
  \end{align}
  #+end_quote
- 誤差の仮定
...
- 母数の推定量
  #+begin_quote
  \begin{align}
    \hat\theta_{1}&=\bar{X}=\frac{1}{n}\sum_{i=1}^{n}X_i\\
    \hat\theta_{2}&=\bar{Y}=\frac{1}{m}\sum_{j=1}^{m}Y_i
  \end{align}
  #+end_quote
- 直感的な議論
  #+begin_quote
  \begin{align}
    |\hat\theta_{1}-\hat\theta_{2}|\text{が小さい}
    &\Rightarrow\text{同じと思って良い}\\
    |\hat\theta_{1}-\hat\theta_{2}|\text{が大きい}
...
- 帰無仮説が正しい場合
  ...
  \begin{equation}
    \hat\theta_{1}-\hat\theta_{2}=\bar{X}-\bar{Y}
  \end{equation}
  は
  \begin{align}
    \text{平均}\quad 
...
- 母数の推定量
  #+begin_quote
  \begin{equation}
    \hat\theta=\bar{X}=\frac{X_{1}+X_{2}+\dots+X_{n}}{n}
  \end{equation}
  #+end_quote
- 推定量の性質
  #+begin_quote
...
- 以下の問に答えよ．
  ...
  それぞれの標本平均を計算したところ
  \begin{align}
    X_{1},X_{2},\dotsc,X_6 & \to \bar{X}=15.2\\
    Y_{1},Y_{2},\dotsc,Y_{10} & \to \bar{Y}=14.5
  \end{align}
  となった．
  本来の平均値が等しいと言えるだろうか？
  #+end_quote
