...
# #+DATE: Time-stamp: <2020-08-25 09:36:56 mura>
:reveal:
...
#+REVEAL_DEFAULT_SLIDE_BACKGROUND: #f4f4fe
#+REVEAL_TITLE_SLIDE_BACKGROUND: #f4f4fe
...
** 試行と見本点
   ...
   - 見本点 (sample point) :: 
     試行の結果観測される
     [[color:darkred][事柄]] \\
     [[color:darkorange][標本，観測値 (observation)，実現値 (realization)]]
     とも呼ばれる
...
** 試行の例
   ...
   - 骰子(サイコロ)振り
     #+begin_quote
     [[color:darkgreen][「骰子を振って，どの目が出易いか調べる」]]
     という実験を考える．
     [[color:darkgreen][「骰子を振ること」]]
     が試行に対応し，
     [[color:darkgreen][1, 2, 3, 4, 5, 6の目]] 
     が見本点となる．
     #+end_quote
   - ルーレット回し
     #+begin_quote
     ...
     また円周の外側の適当な位置に印を付ける．
     [[color:darkgreen][「ルーレットを回して止まったときに印が指している目盛りを読む」]]
     という実験を考える．
     [[color:darkgreen][「ルーレットを回すこと」]]
     が試行に対応し，
     [[color:darkgreen][$(0,1]$ の間のいずれかの実数値]]
     が見本点となる．
     #+end_quote
...
** 見本空間
   - 見本空間 (sample space) ::
     観測される全ての見本点を集めた集合\\
     [[color:darkorange][標本空間]]
     と呼ぶ場合もある
   - 記法 :: 
     - 見本空間 :: $\Omega$ (オメガの大文字)のように大文字
     - 見本点 :: $\omega$  (オメガの小文字)のように小文字
...
** 見本空間の例
   ...
   - 骰子振り
     #+begin_quote
     試行 $T=$ 「骰子を振る」の見本空間は $\Omega=\{1,2,3,4,5,6\}$ であり，
     試行 $T$ の結果2の目が出た場合は
     [[color:darkgreen][「2が観測された」]]
     あるいは
     [[color:darkgreen][「$\omega=2$ が観測された」]]
     という.
     #+end_quote
   - ルーレット回し
     #+begin_quote
...
** 事象の例
   - 骰子振り
     ...
     [[color:darkgreen][「事象 $A$ が起こった」]]
     ことになり，
     5の目が出た場合は $5\not\in A$ なので
     [[color:darkgreen][「事象 $A$ は起こらなかった」]]
     ことになる．
     #+end_quote
   - ルーレット回し
     #+begin_quote
...
** 条件による事象の例
   ...
   - ルーレット回し
     #+begin_quote
     [[color:darkgreen][「 区間 $(0,0.5]$ の数」]]という事象 $A$:
     #+begin_src latex
       \begin{equation}
...
** 有限試行の確率測度
   - 定義
     ...
     *確率測度 (probability measure)*
     ...
     #+end_quote
   - 確率測度は
     [[color:darkorange][確率分布 (probability distribution)]]
     あるいは単に
     [[color:darkorange][分布 (distribution)]]
     と言うこともある．
