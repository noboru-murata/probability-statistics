...
# #+DATE: Time-stamp: <2020-07-20 21:12:23 mura>
:reveal:
...
** 平均値の差の検定
   - 問題
     ...
     #+begin_src latex
       ...
       \end{align}
     #+end_src
     が与えられたとき，その平均値が等しいか検定せよ．
     #+end_quote
   - 検定問題
     #+begin_quote
     それぞれの平均値を $\theta_1,\theta_2$ とすると
     #+begin_src latex
       \begin{equation}
   ...
   - 帰無仮説が正しい場合
     ...
     #+begin_src latex
       \begin{align}
         ...
         E(\bar{X}-\bar{Y})&=0\\
         \text{分散}\quad 
         V(\bar{X}-\bar{Y})&=\left(\frac{1}{n}+\frac{1}{m}\right)\sigma^2
       \end{align}
     #+end_src
     の正規分布に従う．
     #+end_quote
   ...
   - 棄却域の構成
     ...
     #+begin_src latex
       \begin{equation}
         R_{0.05}
         =
         \left(-\infty,-z_{1.96}\right)
         \cup
         \left(z_{1.96},\infty\right)
       \end{equation}
     #+end_src
     #+end_quote
