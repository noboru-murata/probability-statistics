...
# #+DATE: Time-stamp: <2020-06-16 08:38:46 mura>
:reveal:
  ...
  - $Y=\sigma X + \mu$ の関係を利用
    #+begin_src latex
      \begin{align}
        ...
        &=e^{\mu is}\cdot\mathbb{E}[e^{i(s\sigma) X)}]
          =e^{\mu is}\cdot\phi(s\sigma)\\
...
** Kolmogorov の大数の強法則
   - 定理
     ...
     #+end_quote
   #+reveal: split
   - 前出の大数の法則(弱法則)より強い意味での収束性を主張している
     - 大数の弱法則: 確率収束
     - 大数の強法則: 概収束
