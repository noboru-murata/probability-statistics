...
#+DATE:
# Time-stamp: <2022-04-20 17:15:48 mura>
:REVEAL:
...
- 定義
  ...
  #+end_quote
  # #+begin_quote
  # 確率変数は，試行\(T\)の結果として標本点\(\omega\)が出現したとき，
  # その標本点に対応して\(X(\omega)\)という値を取る偶然量を表している．
  # 一般の標本空間は，
  # 骰子の目の系列や工場の製品など，
  # 数値とは限らない要素の集合を考えているため，そのままでは
  # 後に定義する"平均値"や"分散"といった数値と対応づけることが難しい．
  # 例えば"1が出るまで骰子を振り続ける"試行の
  # 標本点として得られる長さの異なる系列に対して
  # "標本点の平均"を定義することはできないだろう．
  # 一方，
  # 確率変数は
...
- [[color:green][骰子振り]]
  #+begin_quote
  "1が出るまで骰子を振り続ける"試行で得られる標本点を \(\omega\) とする．
  このとき，系列 \(\omega\) の長さ
  #+begin_src latex
    \begin{equation}
...
- [[color:green][工場の製品]]
  ...
  最大出力と燃費を測定したとする．
  この場合は標本点 \(\omega\) は生産されるエンジンの個体を表し，
...
- 標本点の写像
  #+begin_quote
  標本空間 \(\Omega^X\) の任意の部分集合 \(B\) に対して
  "\(X\) の値が \(B\) に入る" (\(X\in B\)) という事象を定義する．
  この事象に含まれる元の標本空間 \(\Omega\) の標本点は
  #+begin_src latex
    \begin{equation}
      A
...
# #+begin_quote
# \(X\)は実数値を取るので，
# \(X\)の標本空間\(\Omega^X\)は\(\mathbb{R}\)の部分集合である．
# また連続分布を考える場合，
# 測度の定義域は特に断わらない限り
...
- [[color:green][ルーレット回しの確率変数]]
  ...
  または
  # である．
  # これは \(\omega\) の標本空間を用いた計算として示してあるが，
  # \(X\) の確率空間を考えると
  # その標本空間は \(\Omega^{X}=\left\{0,1\right\}\) であり，
...
- 定義
  ...
  *平均値のまわりのk次モーメント* は
  ...
  で定義される．
  #+end_quote
  - /[[color:orange][分散は平均値のまわりの2次モーメントである]]/
...
- 定義
  ...
  *特性関数* (characteristic function) 
  ...
  で定義される．
  #+end_quote
  - /[[color:orange][特性関数は実数値ではなく複素数値をとることに注意する]]/
