...
# #+DATE: Time-stamp: <2020-06-28 11:30:19 mura>
:reveal:
   ...
   - 平均 $\theta$ , 分散 $\sigma^2$ の正規分布
     - 平均母数 $\theta$ に関するFisher情報量:
       #+begin_src latex
         ...
         \end{equation}
       #+end_src
     - Cramer-Rao下界: $1/nI(\theta)=\sigma^2/n$ 
     - 標本平均 $\bar{X}$ の分散: $\sigma^2/n$ (=Cramer-Rao下界)
   - $\bar{X}$ は平均の一様最小分散不偏推定量

** 一般の分布の場合の注意
   - 標本平均の平均，分散は同様
   - Fisher 情報量は分布によって異なる
   - 必ずしも標本平均が良い不偏推定量になるとは限らない
...
** 解答例
  ...
  - $\varepsilon$ の分散
    #+begin_src latex
      ...
      \end{align}
    #+end_src
  - 密度関数が滑らかでないので Fisher 情報量は定義されない．
  #+reveal: split
